...
&& ret=0
    ...
    (rpc)
		...
		# Note: this matches the last completed word
		# So if you have "tio rpc dev." it will be "rpc"
		case $line[1] in
			(list)
_arguments "${_arguments_options[@]}" : \
'-r+[Sensor root address]:ROOT:_urls' \
...
&& ret=0
;;
# If the last completed word is "rpc" exactly,
# Then it's not an rpc name and we should not trigger this completion
(rpc)
;;
([^-]*)
_arguments "${_arguments_options[@]}" : \
